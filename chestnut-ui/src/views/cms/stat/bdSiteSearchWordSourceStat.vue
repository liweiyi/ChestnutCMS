<template>
  <div class="bdsite-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-radio-group v-model="queryParams.source" size="small">
          <el-radio-button label="indicator">按指标</el-radio-button>
          <el-radio-button label="engine">按搜索引擎</el-radio-button>
        </el-radio-group>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-row :gutter="10" class="mb8">
      <el-card shadow="hover">
        <div slot="header" class="clearfix">
          <span>{{ $t('Stat.Site.SearchWordSource') }}</span>
        </div>
        <el-table v-loading="loading" :data="sourceDataList" size="mini">
          <el-table-column 
            :label="$t('Stat.Site.SearchWord')"
            align="left"
            prop="name" />
          <el-table-column 
...
export default {
  name: "CMSSiteBdSearchWordSource",
  components: {
    LineChart
  },
  data () {
  ...
  methods: {
    loadSiteList() {
      baiduTongjiApi.getSiteList().then(response => {
        ...
        if (this.siteOptions.length > 0) {
          ...
          this.loadSiteSearchWordSourceDatas();
        }
      });
    },
    loadSiteSearchWordSourceDatas () {
      if (this.siteOptions.length == 0) {
      ...
      baiduTongjiApi.getSiteSearchWordSource(this.queryParams).then(response => {
    ...
    resetQuery() {
      ...
      this.queryParams.source = 'indicator';
