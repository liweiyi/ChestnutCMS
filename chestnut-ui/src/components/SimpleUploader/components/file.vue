    ...
    <slot
      ...
      <div class="uploader-file-info">
        ...
        <div class="uploader-file-status">
          <span v-show="status !== 'uploading'">{{ md5Message }} {{statusText}}</span>
        ...
        <div class="uploader-file-actions">
          <span class="uploader-file-pause" v-show="file.paused&&file.md5ing==false" @click="pause"></span>
  ...
  export default {
    ...
    computed: {
      ...
      statusText () {
        ...
        if (typeof fileStatusText === 'function') {
          txt = fileStatusText(status, this.file, this.response)
        } else {
    ...
    watch: {
      status (newStatus, oldStatus) {
        // console.log("file.status", newStatus, oldStatus)
    ...
    methods: {
      ...
      triggerMd5Start() {
        ...
        this.md5Timer = setInterval(() => {
          this.getMd5Processing();
        }, 1000);
      },
      triggerMd5Completed() {
        this.$nextTick(() => {
          clearInterval(this.md5Timer);
          this.md5ing = false;
          this.md5Current = 0;
          this.md5Total = 0;
          this.md5Message = "";
          this.$emit("fileReady", this.file)
        })
      },
      getMd5Processing() {
        this.$nextTick(() => {
          this.md5Message = `[${Math.round(this.md5Current * 100 / this.md5Total, 2)}%]`;
        })
      },
      computeMD5(file) {
        let that = this;
        ...
        fileReader.onload = (e) => {
          ...
          if (currentChunk < chunks) {
            ...
            if (file.aborted) {
              ...
              spark.destroy();
              file.md5ing = false;
              console.log(`第${currentChunk}分片解析中断。`);
              return;
            }
            loadNext();
            this.md5Current = currentChunk;
            // MD5计算进度
