...
<script setup name="CmsHotWordGroupTree">
...
function loadGroupTreeData () {
  getHotWordGroupTreeData().then(response => {
    ...
    nextTick(() => {
      ...
      if (selectedGroupId.value && proxy.$refs.tree) {
        proxy.$refs.tree.setCurrentKey(selectedGroupId.value);
        treeExpandedKeys.value = [selectedGroupId.value];
        emit("node-click", proxy.$refs.tree.getCurrentNode());
      } else {
...
function handleNodeClick (data) {
  ...
  proxy.$cms.setLastSelectedHotWordGroup(selectedGroupId.value);
