...
export function getResponseCodeErrMsg(code, defaultMsg) {
  let errMsg = defaultMsg || i18n.global.t("Error.Unknown")
  if (code == 401) { 
    errMsg = i18n.global.t("Error.Err401") 
  } else if (code == 403) {
    errMsg = i18n.global.t("Error.Err403") 
  } else if (code == 404) {
    errMsg = i18n.global.t("Error.Err404") 
  }
  return errMsg;
}
...
export function isInternalUrl(iurl) {
  return isNotEmpty(iurl) && iurl.startsWith("iurl://");
}

export function getInternalUrlId(iurl) {
  if (!isInternalUrl(iurl)) {
    return null;
  }
  let queryString = iurl.substring(iurl.indexOf('?') + 1);
  let idParam = queryString.split("&").find(item => item.startsWith("id="));
  if (idParam) {
    return idParam.substring(3);
  }
  return null;
}